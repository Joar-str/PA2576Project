...
class MainApp(MDApp):
    ...
    def amount_ad(self):
        ...
        try:
            ad_list = HomePage().get_all_user_ads(HomePage().get_user_id(self.get_name()))
    ...
    def get_ads(self):
        ...
        try:
            ...
            for i in range(len(ad_list)):
                ...
                item.add_widget(icon)
                item.bind(on_press=self.edit_ad_input)
                self.sm.get_screen('home_page').ids.container2.add_widget(item)
